...
```text
Use $cat-note-illustrations 为“AI 帮我把想法摊开检查”生成一张正文配图。
```

如果能生成一张白底、手账贴纸风、黄色竖瞳猫猫参与核心动作的 16:9 配图，就说明 Skill 已经生效。
...
这个 Skill 不是“任何猫猫插画”的通用 prompt。为了保持开源用户生成结果稳定，它默认坚持下面这些边界：

- 不追求写实猫、摄影感或 3D 渲染。
- 不生成圆滚滚 Q 版猫，猫猫应保留三角耳、略笨拙的身体和松弛手绘感。
- 不生成复杂背景海报、商业 KV 或大段文字信息图。
- 不随机更换猫猫 IP，普通任务默认使用 Specimen 0。
- 不把猫猫当装饰物，猫猫必须参与画面里的核心认知动作。
- 不直接复刻参考图或原项目示例图，只复用工作流思路并使用新的猫猫风格系统。
...
欢迎提交更好的 prompt、QA 规则、示例图和使用案例。建议先阅读 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
本项目保留原项目 MIT License notice，并在 [NOTICE.md](NOTICE.md) 中说明改编来源。生成图片是否可商用，取决于你使用的图像模型、平台规则和具体生成结果，请在发布前自行确认。
